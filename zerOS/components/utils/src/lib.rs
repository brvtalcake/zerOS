...
pub trait ToAnyIter<'item>
{
	fn to_any_iter<'collection>(
		&'collection self
	) -> impl Iterator<Item = &'item dyn Any> + 'collection;
}

pub trait ToAnyIterMut<'item>
{
	fn to_any_iter_mut<'collection>(
		&'collection mut self
	) -> impl Iterator<Item = &'item mut dyn Any> + 'collection
	where
		'collection: 'item;
}

impl<'item, C, I> ToAnyIter<'item> for C
where
	for<'collection> &'collection Self: IntoIterator<Item = &'collection &'item I>,
	I: Any
{
	fn to_any_iter<'collection>(
		&'collection self
	...
	{
		self.into_iter().map(move |elem| (*elem) as &dyn Any)
	}
}

impl<'item, C, I> ToAnyIterMut<'item> for C
where
	for<'collection> &'collection mut Self: IntoIterator<Item = &'collection mut &'item mut I>,
	I: Any
{
	fn to_any_iter_mut<'collection>(
		&'collection mut self
	...
	{
		self.into_iter()
			.map(move |elem| ((*elem) as &mut I) as &mut dyn Any)
	}
}
...
pub trait IsTrueVal: Bit
{
	type Value: Bit = False;
}

impl IsTrueVal for False {}
