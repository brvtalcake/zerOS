...
pub fn main() {
    ...
    println!("cargo::rerun-if-changed=linker-x86_64.ld.template");
...
{
    let relrsfiles = [ "./src/entry-point", "./src/kernel/map" ];
    let modrsfile: PathBuf;
    let rsfiles: OsString = match relrsfiles.map(realpath) {
        [Ok(path1), Ok(path2)] => {
            let mut ret = path1.join("linker_output.rs").into_os_string();
            ret.push(",");
            modrsfile = path2.join("public_generated.rs");
            ret.push(path2.join("__generated.rs"));
            ret
        },
        [Err(e), _     ] => panic!("can not find {}: {}", relrsfiles[0], e.to_string()),
        [_     , Err(e)] => panic!("can not find {}: {}", relrsfiles[1], e.to_string()),
    };
    let rsfiles: String = rsfiles.into_string().expect("invalid path string");
